...
public class VoteNormalManageProcessor implements Vote_GeneralMethods {
	public final Main plugin;
	protected VoteNormalSystem voteNormalSystem;
	public VoteNormalManageProcessor(Main plugin) {
		this.plugin = plugin;
		voteNormalSystem = new VoteNormalSystem(plugin);
		init();
	}
	protected void init() {
		plugin.registerCommandProcessMethod("vote-normal-manage", (plugin, sender, command, label, args) -> {
			...
			if(argLength == 0) {
				...
				if(arg0.equalsIgnoreCase("help")) {
				...
				else if(arg0.equalsIgnoreCase("settting")) {
					if(argLength == 1) {
						return false;
					}
					else {
					...
					if(argLength == 1) {
						...
						for(String voteName : voteNormalSystem.ongoingVotes.keySet()) {
							...
							sender.sendMessage("  " + voteName + ": " + numPlayers + " casted " + numCasts + " votes with " + numSelections + " available selections.");
						}
						return true;
					}
					else {
						...
						if(!voteNormalSystem.ongoingVotes.containsKey(arg1)) {
							...
							if(voteNormal == null) {
								...
								for(String selectionName : voteNormal.selections.keySet()) {
									...
									sender.sendMessage("  Selection: " + selectionName);
									sender.sendMessage("    total casts: " + selectionTotalCasts);
									for(UUID uuid : selection.voteCount.keySet()) {
										Integer playerCasts = selection.voteCount.get(uuid);
										if(playerCasts == null) {
											continue;
										}
